...
class FashionSave(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.method == 'GET':
                ...
                # context={
                #     'data': fashion_item_data,
                #     'fashion': fashion_item_data
                # }
...
class FashionCustomizeEdit(View):
    def get(self, request, *args, **kwargs):
        if request.method == 'GET':
            ...
            brand_tag = ''
            # tag_count = 1
            # if fashion_list_data.count() > 0:
            #     tag_count = fashion_list_data.count()
            
            for split_value in fashion_list_data:
                # if split_value.brand_tag.upper():
                brand_tag += split_value.using_item + ','

            # request.session['FashionItemData'] = brand_tag
            # print( brand_tag )
            # print (fashion_list_data)
...
class webcam(View):
    def get(self, request, *args, **kwargs):
        context={'image': False}
        return render(request, 'app/test/webcam.html', context)
...
# def webcam(request):
#     return render(request, 'app/test/webcam.html')
